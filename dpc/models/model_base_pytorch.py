...
def pool_single_view(cfg, tensor, view_idx):
    ...
    return tf.gather_nd(tensor, indices)tensor[indices.transpose(0,1).long().numpy().tolist()]
...
class ModelBase(object):  # pylint:disable=invalid-name
    ...
    def preprocess(self, raw_inputs, step_size, random_views=True):
        ...
        inputs['masks'] = raw_inputs['mask'][indices.transpose(0,1).long().numpy().tolist()]
        inputs['images'] =raw_inputs['image'][indices.transpose(0,1).long().numpy().tolist()]

        if cfg.saved_depth:
            # inputs['depths'] = tf.gather_nd(raw_inputs['depth'], indices)
            inputs['depths'] = raw_inputs['depth'][indices.transpose(0, 1).long().numpy().tolist()]
