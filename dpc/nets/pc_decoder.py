...
def model(inputs, outputs_all, cfg, is_training):
    with tf.variable_scope('point_cloud', reuse=tf.AUTO_REUSE):
        ...
        extra_layer_dim = 4096
        pts_tmp = slim.fully_connected(inputs, extra_layer_dim,
                                    activation_fn=tf.nn.leaky_relu,
                                    weights_initializer=w_init)
        pts_raw = slim.fully_connected(pts_tmp, num_points * 3,
                                    activation_fn=None,
                                    weights_initializer=w_init)
